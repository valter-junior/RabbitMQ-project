...
func fuzzyficationMsgSecInput(msgSec float64) map[string]float64 {
	...
	peaks := []float64{
		-10000, // Muito Grande Negativo
		-5000,  // Grande Negativo
		-1000,  // Pequeno Negativo
		0,      // Zero
		1000,   // Pequeno Positivo
		5000,   // Grande Positivo
		10000,  // Muito Grande Positivo
	}
	...
	fuzzy[Zero] = gaussianMembership(msgSec, peaks[3], sigmas[3])
	fuzzy[MediumPositive] = gaussianMembership(msgSec, peaks[4], sigmas[4])
	fuzzy[LargePositive] = gaussianMembership(msgSec, peaks[5], sigmas[5])
	fuzzy[VeryLargePositive] = gaussianMembership(msgSec, peaks[6], sigmas[6])
...
func fuzzyficationOutput(n float64) map[string]float64 {
	...
	// Redefinindo os picos para cobrir o intervalo de -8 a 8
	peaks := []float64{
		-8, // Grande Diminuição
		-4, // Média Diminuição
		-2, // Pequena Diminuição
		0,  // Manter
		2,  // Pequeno Aumento
		4,  // Médio Aumento
		8,  // Grande Aumento
	...
	// Calculando sigmas considerando que o intervalo entre os picos é maior
	sigmas := make([]float64, len(peaks))
	for i := range peaks {
		if i == len(peaks)-1 {
			// Aumenta o sigma em 1.5 vezes para os extremos
	...
	// Aplicando a função de pertinência gaussiana para cada categoria
	...
	r[Maintain] = gaussianMembership(n, peaks[3], sigmas[3])
	r[MediumIncrease] = gaussianMembership(n, peaks[4], sigmas[4])
	r[LargeIncrease] = gaussianMembership(n, peaks[5], sigmas[5])
	r[VeryLargeIncrease] = gaussianMembership(n, peaks[6], sigmas[6])
...
func main() {
	...
	file, err := os.OpenFile("message_rate_gaussian_7.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		log.Fatal(err)
	}
	defer file.Close()
