...
func fuzzyficationMsgSecInput(msgSec float64) map[string]float64 {
	...
	peaks := []float64{
		-10000, -5000, -2500, -1250, -625, 0, 625, 1250, 2500, 5000, 10000,
	}
	...
	fuzzy[VerySmallNegative] = gaussianMembership(msgSec, peaks[4], sigmas[4])
	fuzzy[Zero] = gaussianMembership(msgSec, peaks[5], sigmas[5])
	fuzzy[VerySmallPositive] = gaussianMembership(msgSec, peaks[6], sigmas[6])
	fuzzy[SmallPositive] = gaussianMembership(msgSec, peaks[7], sigmas[7])
	fuzzy[MediumPositive] = gaussianMembership(msgSec, peaks[8], sigmas[8])
	fuzzy[LargePositive] = gaussianMembership(msgSec, peaks[9], sigmas[9])
	fuzzy[VeryLargePositive] = gaussianMembership(msgSec, peaks[10], sigmas[10])
...
func fuzzyficationOutput(n float64) map[string]float64 {
	...
	r[VeryLargeIncrease] = gaussianMembership(n, 8, sigmas[10])
...
func main() {
	...
	file, err := os.OpenFile("message_rate_gaussian_11.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		log.Fatal(err)
	}
	defer file.Close()
